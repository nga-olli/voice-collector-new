export const state = () => ({
  ...
  export const actions = {
    async get_all({ commit }, { query }) {
      return await this.$axios.$get(`/v1/jobs`, {
        params: query
      }).then(res => {
        commit('SET_DATA', res)
    ...
    async add({ commit }, { formData, covers }) {
      ...
      return await this.$axios.$post(`/v1/jobs`, data);
    },
  
    async get_form_source({ commit }) {
      return await this.$axios.$get(`/v1/jobs/formsource`)
        .then(res => commit('SET_FORM_SOURCE', res));
    },
  
    async bulk({ commit }, { formData }) {
      return await this.$axios.$post(`/v1/jobs/bulk`, formData);
    },
    ...
    async delete({ commit }, { id }) {
      return await this.$axios.$delete(`/v1/jobs/${id}`)
