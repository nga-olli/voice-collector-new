...
export const actions = {
  async get_all({ commit }, { query }) {
    return await this.$axios.$get(`/v1/rewards/category`, {
      params: query
    }).then(res => {
      commit('SET_DATA', res)
  ...
  async get_all_closure({ commit }, { query }) {
    return await this.$axios.$get(`/v1/rewards/category/closure`)
      .then(res => {
        commit('SET_DATA', res)
      });
  },
  ...
  async add({ commit }, { formData, covers }) {
    ...
    return await this.$axios.$post(`/v1/rewards/category`, data);
  },

  async get_form_source({ commit }) {
    return await this.$axios.$get(`/v1/rewards/category/formsource`)
      .then(res => commit('SET_FORM_SOURCE', res));
  },

  async bulk({ commit }, { formData }) {
    return await this.$axios.$post(`/v1/rewards/category/bulk`, formData);
  },

  async get({ commit }, { id }) {
    return await this.$axios.$get(`/v1/rewards/category/${id}`);
  },
  ...
  async update({ commit }, { id, formData, covers }) {
    ...
    return await this.$axios.$post(`/v1/rewards/category/${id}`, data)
      .then(res => commit('UPDATE_DATA', res.data));
  },

  async delete({ commit }, { id }) {
    return await this.$axios.$delete(`/v1/rewards/category/${id}`)
